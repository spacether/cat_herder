...
class TiltedRect {
  ...
  cornerInOther(lines, otherLines){
    let foundPts = [];
    ...
    if (foundPts.length > 0) {
      ...
      if(foundPts.length === 2) {
        ...
        let newPt = foundPts[0].add(foundPts[1]).mult(0.5);
        return newPt;
  ...
  findHit(other){
    ...
    if (other.constructor.name === 'TiltedRect') {
      ...
      let result = this.otherInThis(other);
      // this has a corner in other
      if (!result) result = this.cornerInOther(lines, otherLines);
...
//   hit = collidePointPoly(x, y, vects);
  ...
  normal(pt){
    ...
    for (let i = 0; i < pts.length; i++) {
      if (pts[i].equals(pt)) {
        console.log('detected hit on corner point');
...
class Line {
  ...
  hits (other) {
    ...
    if (other.constructor.name === 'Vector') {
      let {x, y} = other;
      hit = collidePointLine(x, y, p1.x, p1.y, p2.x, p2.y, 2);
    }
    return hit;
  }
  render() {
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__actors_js__ = __webpack_require__(3);
