...
class TiltedRect {
  ...
  hits(other){
    ...
    if (other.constructor.name === 'TiltedRect') {
      ...
      let dummyNormal = new __WEBPACK_IMPORTED_MODULE_0__vector_js__["a" /* default */](0, 0);
...
class Line {
  ...
  hits (other) {
    ...
    if (other.constructor.name === 'Vector') {
      ...
      if (result.x !== false) hit = new __WEBPACK_IMPORTED_MODULE_0__vector_js__["a" /* default */](result.x, result.y);
    }
    return hit;
  }
  render() {
...
class Level {
  constructor({x, y, width, height, playAudio, moveItems, win, numCats}){
    ...
    // player set-up
    let player = new __WEBPACK_IMPORTED_MODULE_2__player_js__["a" /* default */](700, 200, 20, 20*2.2, this.sounds['CALL_CATS'], playAudio, this.drawable);
  ...
  createCats(numCats){
    ...
    for(let i=0; i<numCats; i++){
      ...
      while (hit){
        ...
        cat = new __WEBPACK_IMPORTED_MODULE_1__cat_js__["a" /* default */](i, loc.x, loc.y, 16, 30, this.sounds['CAT_MEW'], this.sounds['CAT_ANGRY'], this.playAudio);
        hit = false;
...
class Cat {
  constructor (id, x, y, width, length, mewSound, angrySound, playAudio) {
  ...
  unfreeze(){
    if (!this.canMove) {
      this.playAudio(this.angrySound);
      this.reverse();
      this.canMove = true;
      this.adder = 15;
    }
  ...
  getHits(hitable){
    ...
    for (let i = 0; i < hitable.length; i++) {
      ...
      if (this !== other){
        let hitData = this.hits(other);
        if (hitData) {
          if (hitData === true || hitData.constructor !== Array) {
            debugger; // handle fail case
          }
